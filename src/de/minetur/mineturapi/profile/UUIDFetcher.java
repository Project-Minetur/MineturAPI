...
public class UUIDFetcher {
	...
	private static Gson gson = new GsonBuilder().registerTypeAdapter(UUID.class, new UUIDType()).create();
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(
					String.format("https://api.mojang.com/users/profiles/minecraft/%s?at=%d",
							new Object[] { name, Long.valueOf(timestamp / 1000L) })).openConnection();
	...
	public static String getName(UUID uuid) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(String
					.format("https://api.mojang.com/user/profiles/%s/names", new Object[] { UUIDType.fromUUID(uuid) }))
							.openConnection();
			connection.setReadTimeout(5000);
